...
#Optimizers
optimizer_F = torch.optim.Adam(list(F1.parameters()) + list(F2.parameters()), lr=opt.learning_rate, betas=(0.5, 0.999))
...
# define name
name = 'mgantri_rev_banana_l' + str(mon_lambda)[2:]
...
for ep in range(epochs):
    ...
    for y in train_loader:
        ...
        Fz = torch.cat((F1(z2[:,0].unsqueeze(1)), F2(z2)), 1)
        ...
        Fz_prime = torch.cat((F1(z2_prime[:,0].unsqueeze(1)), F2(z2_prime)), 1)
    ...
    with torch.no_grad():
        Fz = torch.cat((F1(zt[:,0].unsqueeze(1)), F2(zt)), 1)
    ...
    with torch.no_grad():
        Fz = torch.cat((F1(z_t[:,0].unsqueeze(1)), F2(z_t)), 1)
    ...
    #true_pdf   = ban.prior_pdf(torch.fliplr(y_train_test).numpy())
...
#Save model
torch.save(F1.state_dict(), name + '_F1.pt')
torch.save(F2.state_dict(), name + '_F2.pt')
