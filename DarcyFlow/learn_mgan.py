...
import sys
sys.path.append('../')
from utilities import MatReader, UnitGaussianNormalizer, FCFFNet
...
parser.add_argument("--std_y", type=float, default=0.02, help="standard deviation of observational noise")
...
#Read raw data (note: reverse order)
dataset = 'GenerateData/darcy_data_noiseless_latentdim'+str(args.latent_dim)
reader = MatReader(dataset+'.mat')
x = reader.read_field('x_score')
y = reader.read_field('y')
#Check number of inputs
if x.shape[0] != y.shape[0]:
    raise ValueError('Data doesn''t have correct dimensions')
if x.shape[0] < args.n_train:
    raise ValueError('Not enough samples')
#Subselect data and reshape inputs
...
# add noise
y_train += args.std_y*torch.randn(y_train.shape)

# save training data
sio.savemat(dataset+'_stdy'+str(args.std_y)+'.mat',{'x_train':x_train.detach().numpy(),'y_train':y_train.detach().numpy()})
...
# define filename
name = 'mgan_' + dataset + '_mon' + str(args.m_lambda)[2:] + '_stdy' + str(args.std_y)[2:]
...
plt.ylim(-1.5,1.5)
plt.savefig('gradient_penalty_'+name+'.pdf')
plt.close()
...
plt.savefig('monotonicity_penalty_'+name+'.pdf')
plt.close()
